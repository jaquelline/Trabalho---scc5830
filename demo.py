...
def encodingDCT(img, text):
	...
	for x in range(1, matrixW):
		for y in range(1, matrixH):
			...
			if (c1 > c2):
				c1 = c1 + ((valueForce - (c1 - c2))/2)
				c2 = c2 - ((valueForce - (c1 - c2))/2)
			else:
				c1 = c1 - ((valueForce - (c2 - c1))/2)
				c2 = c2 + ((valueForce - (c2 - c1))/2)
...
def chiSquare(im1, im2):
	...
	if (chisqr < 3.841):
		return 'The value of chi square is ' + str(chisqr) + '. The image is not suspested!'
	else:
		return 'The value of chi square is ' + str(chisqr) + '. The image is suspested!'
    return chisqr
...
cv2.imwrite('stego.jpg', stego, [cv2.IMWRITE_JPEG_QUALITY, 100])

cv2.imwrite('stego.png', stego, [cv2.IMWRITE_PNG_COMPRESSION, 0])
